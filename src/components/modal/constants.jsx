...
const UpdateTradeURL = {
		"csgo": (value) => updateTradeURL((tradeURLs) => tradeURLs = {...tradeURLs, steam: value}),
		"dota2": (value) => updateTradeURL((tradeURLs) => tradeURLs = {...tradeURLs, steam: value})
}

const CheckUndefinedNull = (value, replacement) => 
		(value === undefined || value === null) ? replacement : value;
...
const updateTradeURL = (updateUrl) => {
		...
		localStorage.setItem("trade-urls", JSON.stringify(tradeURLs));
};
...
const Constants = {
		Regex, TradeURL, IsRegexTradeURL, 
		UpdateTradeURL, CheckUndefinedNull, WithdrawErrors
};
